...
if [ ! -e pa_stable_$VERSION.tgz ]; then
    ...
    if [ -d "$DOWNLOAD_DIR" ]; then
        ...
        $WGET -T 10 -t 3 http://files.portaudio.com/archives/pa_stable_candidate_$VERSION.tgz
    fi

    if [ ! -e pa_stable_candidate_$VERSION.tgz ]; then
        echo "Download of pa_stable_candidate_$VERSION.tgz - failed!"
        echo "Aborting script. Please download and install port audio manually!"
        exit 1;
    fi
fi

tar -xovzf pa_stable_candidate_$VERSION.tgz || exit 1
